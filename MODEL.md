...
- `quote` 逐字照抄來源，含來源自己的 ≧／≥、mg/dl／mg/dL 不一致；**不美化、不翻譯、不合併兩段**（不連續的原文用 `quote_extra`）。
  - ⚠️ **「逐字」包含來源自己的句尾標點**：來源句子結尾有句號就要有，不要因為看起來多餘而拿掉。
    ☠️ **收據 gate 擋得住「竄改」，擋不住「截斷」**——`check-receipts.py` 用的是
    `if norm(quote) in hay`（子字串包含，`norm` 只吃空白），而**截斷後的引句仍然是快照的子字串**，
    所以少抄句尾標點、或從句子中間切掉後半，gate 一律綠燈。這不是 gate 寫壞，是子字串比對的先天射程。
    ⇒ 落盤時**從機器可讀的原始檔取字串**（JSON 欄位／原始 HTML），不要人工複製；
    需要更硬的保證時才加「引句必須以來源的句子邊界結束」這類檢查，屬另一輪。
  - 引用前先分類再決定標點：**完整句**帶來源句尾標點／**片段引用**不帶／**句中截斷**優先補完整句而不是加刪節號。
